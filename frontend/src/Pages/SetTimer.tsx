...
// useEffect(() => {
//   console.log(interval,shouldBreak);
  
// },[interval,shouldBreak])
  ...
  return (
    <section className={styles.wrapperSetTimer}>
      <div className={styles.wrapTime}>
        <img onClick={reduceTime} src="https://cdn-icons-png.flaticon.com/512/271/271220.png" alt="" /> <span className={styles.timeCount}>{timer}</span>{" "}
      ...
      <div className={styles.wrapInput}>
        <label htmlFor="">
          <input onChange={(e) => {
            ...
            setInterval(e.target.checked)}} type="checkbox" />
          intervals
        </label>
        <label htmlFor="">
          <input onChange={(e) => { setShouldBreak(e.target.checked)}} type="checkbox" />5 min break / interval{" "}
        </label>
      </div>

      <button onClick={setTime} className={styles.startBtn}>START TIMER</button>
      <AbortBtn />
    </section>
  );
}
